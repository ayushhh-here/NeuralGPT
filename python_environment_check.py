# Copyright (c) Sebastian Raschka, under the Apache License 2.0.
# (This repo's own LICENSE file is MIT; this file is a copy from the source
# below and keeps the original Apache-2.0 license, per the README.)
...
if version_parse(platform.python_version()) < version_parse("3.10"):
    print("[FAIL] This project requires Python 3.10 or newer but found version %s" % sys.version)
else:
...
def check_cuda():
    """
    Reports whether PyTorch can see a CUDA-capable GPU. This is informational
    only (CPU-only setups are fully supported, just slower) so it never
    fails the check - it just tells you what device training will use.
    """
    try:
        torch = import_module("torch")
    except ImportError:
        print("[INFO] torch is not installed yet, skipping CUDA check")
        return

    if torch.cuda.is_available():
        print(f"[OK] CUDA is available (device: {torch.cuda.get_device_name(0)})")
    else:
        print("[INFO] CUDA is not available - training/inference will run on CPU")
